...
const typeLabels: Record<string, { label: string; icon: typeof BookOpen; colorClass: string }> = {
  arcade_game: { label: "Arcade Game · 3 pontos", icon: Gamepad2, colorClass: "text-secondary" },
  ...
  course_long: { label: "Curso 60+ · 2 pontos", icon: BookOpen, colorClass: "text-accent" },
};
...
const BadgeDetailModal = ({ badge, open, onOpenChange }: BadgeDetailModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border sm:rounded-2xl max-w-sm p-0 overflow-hidden shadow-xl">
        <div className="flex items-center justify-center pt-8 pb-4 bg-muted/30">
          <div className="w-28 h-28 rounded-2xl bg-card flex items-center justify-center shadow-md">
            {badge.image.startsWith("http") ? (
              <img src={badge.image} alt={badge.name} className="w-24 h-24 object-contain" />
            ) : (
          ...
          <div className="space-y-3">
            ...
            {badge.link && (
              <a
                ...
                className="flex items-center gap-3 bg-muted/30 rounded-xl px-4 py-3 border border-border hover:border-primary/30 hover:shadow-md transition-all duration-300 group"
              >
                <ExternalLink className="w-4 h-4 text-primary flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-muted-foreground font-body">Ver no Google Skills</p>
                  <p className="text-sm font-medium text-primary font-body truncate">
                    Abrir badge
                  </p>
                </div>
              </a>
